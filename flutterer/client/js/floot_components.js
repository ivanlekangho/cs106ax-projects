...
function Floot(flootInfo, selectedUser, showDelete, actions) {
    ...
    timestamp.classList.add("floot-timestamp");

    let text = document.createTextNode(timeAgo(flootInfo.timestamp));
    ...
    function timeAgo(timestampStr) {
        let now = new Date();
        let time = new Date(timestampStr);
    ...
    function deleteFloot() {
        ...
        // this floot.
        actions.deleteFloot(flootInfo, selectedUser);
    }
    ...
    function toggleLike(e) {
        ...
        if (flootInfo.liked_by.includes(selectedUser)) {
            actions.unlikeFloot(flootInfo);
        } else {
            actions.likeFloot(flootInfo);
        }
    }
...
function LikeCommentCount(flootInfo, selectedUser, onLike) {
    ...
    likeContainer.classList.add("like-count");
    let heart = MaterialIcon("favorite");
    heart.classList.add("like-heart");
    if (flootInfo.liked_by.includes(selectedUser)) {
        heart.classList.add("liked-heart");
    } else {
        heart.classList.add("unliked-heart");
    }
    likeContainer.appendChild(heart);

    let numLikes = flootInfo.liked_by.length;
    let likeText = document.createTextNode(numLikes);
    likeContainer.appendChild(likeText);

    likeContainer.addEventListener("click", onLike);
