...
function CommentList(floot, loggedInUsername, actions) {
    ...
    for (let commentID of Object.keys(floot.comments)) {
        ...
        container.appendChild(Comment(comment.id, comment.username, comment.message,
            /* showDelete = */ loggedInUsername === comment.username, floot.id, actions, comment.liked_by));
    }
 ...
 *   </div>
 */
function Comment(id, name, message, showDelete, flootId, actions, likedBy) {
    let container = document.createElement("div");
    ...
    function deleteComment() {
        ...
        // this comment.
        actions.deleteComment(id, flootId)
    }

    // EXTENSION: Comment likes
    let likeBox = document.createElement("div");
    likeBox.classList.add("comment-like-count");

    let heart = MaterialIcon("favorite");
    heart.classList.add("comment-heart");

    if (likedBy.includes(actions.selectedUser)) {
        heart.classList.add("comment-liked");
    } else {
        heart.classList.add("comment-unliked");
    }

    likeBox.appendChild(heart);

    let count = document.createTextNode(likedBy.length);
    likeBox.appendChild(count);

    function clickAction(e) {
        if (likedBy.includes(actions.selectedUser)) {
            actions.unlikeComment(flootId, { id, liked_by: likedBy });
        } else {
            actions.likeComment(flootId, { id, liked_by: likedBy });
        }
    }

    likeBox.addEventListener("click", clickAction);
...
function NewCommentEntry(floot, actions) {
    ...
    function submitComment() {
        ...
        // this new comment.
        actions.createComment(textbox.value, floot)
    }
