...
def get_floots():
    ...
    """
    result = []
    floot_objects = db.get_floots()
    for floot in floot_objects:
...
def get_floot(floot_id):
    ...
    if db.has_floot(floot_id):
        ...
        return HTTPError(404, "Floot ID could not be found")
...
def create_floot(request_body):
    ...
    if "message" not in request_body:
        return HTTPError (400, "No message provided")
    if "username" not in request_body:
        return HTTPError (400, "No username provided")
    
    message = request_body["message"]
    username = request_body["username"]
    new_floot = Floot(message, username)
    db.save_floot(new_floot)
    return new_floot.to_dictionary()
...
def delete_floot(floot_id, request_body):
    ...
    * If everything worked fine and the floot was successfully deleted, this
    ...
    if not db.has_floot(floot_id):
        return HTTPError(404, "Floot ID invalid")
    
    if "username" not in request_body:
        return HTTPError(400, "No username provided")
    
    deleter = request_body["username"]
    orig_poster = db.get_floot_by_id(floot_id).get_username()

    if deleter != orig_poster:
        return HTTPError(401, "This floot is not yours, you cannot delete it!")
...
def delete_comment(floot_id, comment_id, request_body):
    ...
    * Otherwise, if everything works successfully, return "OK".
    ...
    for item in floot.get_comments():
        if item.get_id() == comment_id:
            comment = item
            break
    
    if comment == None:
        return HTTPError(404, "Comment does not exist")
    
    if "username" not in request_body:
        return HTTPError(400, "No username provided")
    
    deleter = request_body["username"]
    orig_commenter = comment.get_author()

    if deleter == orig_commenter:
        floot.delete_comment(comment, deleter)
        return "OK"
    
    return HTTPError(401, "This comment is not yours, you cannot delete it!")

#EXTENSION: FLOOT LIKES
...
def like_floot(floot_id, request_body):
    ...
    #EXTENSION
    if not db.has_floot(floot_id):
        return HTTPError(404, "Floot not found")

    if "username" not in request_body:
        return HTTPError(400, "Missing username")
...
#EXTENSION: COMMENT LIKES
#POST /api/floots/<floot_id>/comments/<comment_id>/like
def like_comment(floot_id, comment_id, request_body):
    # check floot exists
    if not db.has_floot(floot_id):
    ...
    # find the comment
    comment = floot.get_comment_by_id(comment_id)
    if comment is None:
        return HTTPError(404, "Comment does not exist")
...
#POST /api/floots/<floot_id>/comments/<comment_id>/unlike
def unlike_comment(floot_id, comment_id, request_body):
    if not db.has_floot(floot_id):
        return HTTPError(404, "Invalid floot ID")
...
POST_ROUTES = [
    ...
    #EXTENSION: Comment Likes
    (("/api/floots/(.*?)/comments/(.*)/like", "floot_id", "comment_id"), like_comment),
    (("/api/floots/(.*?)/comments/(.*)/unlike", "floot_id", "comment_id"), unlike_comment)
]
