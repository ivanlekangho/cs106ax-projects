...
class FlootComment:
    ...
    def __init__(self, message, author, comment_id=None):
        ...
        # Students should NOT use these options to set comment_id.
        if not comment_id:
    ...
    def to_dictionary(self):
        ...
        return {
            ...
            #EXTENSION
            self.COMMENT_LIKES: self.liked_by
        }
    
    def like(self, username):
        if username not in self.liked_by:
            self.liked_by.append(username)

    def unlike(self, username):
        if username in self.liked_by:
            self.liked_by.remove(username)
